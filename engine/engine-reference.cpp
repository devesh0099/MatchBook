// engine-reference.cpp — the reference engine as a submittable engine.cpp.
//
// One translation unit against the frozen headers only (SPEC §4), i.e. the
// oracle's logic flattened out of reference/{reference.h,reference.cpp} with
// the generator-only helpers (is_resting, level_count) dropped. Paste into the
// editor buffer, or `make test` against it locally.
//
// It will pass correctness by construction. It will NOT rank well: std::map of
// std::list is the clarity-first layout the reference deliberately uses.
...
class Engine final : public IMatchingEngine {
 public:
  void on_new(const Order& o, OutSink& sink) noexcept override {
    ...
    // SPEC F2: the fillability check happens BEFORE any mutation and before
    // the Ack, because an unfillable FOK emits only the Reject (A3).
    if (o.tif == TIF::FOK) {
    ...
    switch (o.tif) {
      ...
      case TIF::FOK:
        // Unreachable by F3: the fillability walk counted at least o.qty of
        // other-firm liquidity, and STP cancellations cannot eat into it.
        break;
    }
  }
  ...
  void on_cancel(OrderRef ref, uint64_t seq, OutSink& sink) noexcept override {
    ...
    if (it == index_.end()) {
      // SPEC §1.4: unknown or already-filled — indistinguishable without a
      // graveyard, and not required to be.
 ...
 private:
  ...
  // Bids best-first is highest price first; asks best-first is lowest first.
  using BidsBook = std::map<int32_t, Level, std::greater<int32_t>>;
  using AsksBook = std::map<int32_t, Level, std::less<int32_t>>;

  // Cancel identity is the (session_id, client_order_id) PAIR (SPEC §1).
  ...
  // SPEC F1: read-only walk, best price first, bounded by the limit price,
  // summing ONLY other firms' resting quantity. Mutates nothing.
  template <class Book>
  ...
  // Walks the opposite book best price first, FIFO within each level,
  // emitting Trades and STP CancelAcks in book-walk order (SPEC S5).
  template <class Book>
  void match(Book& book, const Order& o, uint32_t& remaining, OutSink& sink) noexcept {
    ...
    while (remaining > 0 && lit != book.end()) {
      ...
      while (remaining > 0 && oit != level.end()) {
        ...
        // remaining quantity, aggressor continues unchanged.
        ...
        // SPEC §1.1: trade price is the RESTING order's price, always.
