...
1. **I1.** IOC matches under S1–S5 like any aggressor. Whatever remains after the walk,
   **including** quantity that met only same-firm liquidity and was cancelled, emits one
   `Expired`.
...
Warm-up is sized from the stream's own profile: the ranked profile builds a deep book of
roughly **750,000 resting orders across thousands of price levels** before timing begins.
Those warm-up events run through your engine (the digest covers them) but are not
measured, so every ranked sample is taken against a book at full depth.
...
|---|---|---|---|
| **Run** (visible tests) | the visible tests | seconds | unlimited |
| **Submit** → correctness | hidden differential check | seconds | one at a time |
| **Submit** → benchmark | `cancel_heavy` latency benchmark | a couple of minutes | one at a time |

Only **one evaluation of yours runs at a time.** A **Run** (the visible tests) is always
available and unlimited. A **Submit** starts an evaluation, and if you submit again while
a previous one is still in flight, the new submission is **rejected immediately** with a
clear message telling you to wait. It is never queued, and it never cancels the running
job. Nothing is ever silently dropped, so once the current evaluation finishes you simply
resubmit.
...
in absolute terms, since it is a different machine, but they rank your own changes
correctly, which is what iteration needs.
